...
async function findAll(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ message: "No se encontraron ususarios", data: error });
  }
}
...
async function findOne(req: Request, res: Response) {
  try {
    ...
    const usuarioBuscado = await em.findOneOrFail(
      Usuario,
      { id },
      { populate: ["tipoUsuario"] }
    );
    res
      .status(200)
      .json({ message: "Usuario encontrado", data: usuarioBuscado });
  } catch (error: any) {
...
async function login(req: Request, res: Response) {
  try {
    ...
    if (usuario) {
      ...
      res
        .status(200)
        .json({ message: "Logueado correctamente", token: accessToken });
    } else {
    ...
    res
      .status(500)
      .json({ message: "Ocurrió un error inesperado", data: error });
  }
}
...
async function signup(req: Request, res: Response) {
  try {
    ...
    if (isSQLError(error)) {
      res
        .status(500)
        .json({ message: getSQLErrorMessage(error, "Usuario"), data: error });
    } else {
      res.status(500).json({
        message: "No se pudo completar el registro de usuario",
        data: error,
      });
    }
  }
...
async function update(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ message: "No se pudo actualizar el ususario", data: error });
  }
}
...
async function remove(req: Request, res: Response) {
  try {
    ...
    res
      .status(500)
      .json({ message: "No se pudo eliminar el usuario", data: error.message });
  }
}
